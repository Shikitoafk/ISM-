  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-200 ${
        isScrolled
          ? "bg-white/95 backdrop-blur-md border-b-2 border-slate-900 shadow-sm py-3"
          : "bg-white/90 backdrop-blur-sm border-b border-slate-300 py-4"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
        <a href="#" className="flex items-center gap-2 focus:outline-none rounded-md shrink-0">
          <Logo size="md" />
        </a>
        ...
        <nav className="hidden xl:flex items-center gap-5 xl:gap-7" aria-label="Main Navigation">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="text-xs sm:text-sm font-bold text-slate-800 hover:text-brand-800 transition-colors py-1 focus:outline-none whitespace-nowrap"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="hidden md:flex items-center gap-3 shrink-0">
          ...
          <div className="inline-flex items-center rounded-lg border-2 border-slate-900 p-0.5 bg-slate-100 text-xs font-bold shadow-xs">
            {languages.map((l) => (
              <button
                key={l}
                type="button"
        ...
        <button
          ...
          className="xl:hidden p-2 text-slate-900 hover:text-brand-800 focus:outline-none rounded-md"
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="xl:hidden bg-white border-b-2 border-slate-900 px-4 pt-3 pb-6 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-2">
            <div className="inline-flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg border-2 border-slate-900 text-xs font-bold text-slate-900 bg-slate-100">
